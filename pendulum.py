...
def make_pyomo_model(EOM: list, nfe: int, angles: str,
    ...
    # set starting point
    for link in m.links:
        m.q[1, ncp, link].fix(0)
        m.dq[1, ncp, link].fix(0)
...
def make_animation(th, lengths, masses, Tc, num_links: int, nfe: int,
    ...
    def animate(i):
        ...
        for j in range(1, num_links):
            ...
            # mass_circs[j].center = (x[j], y[j])
            rods[j].set_data([x[j], x[j-1]], [y[j], y[j-1]])

            #Tc_circs[j].center = (x[j-1], y[j-1])
            #Tc_circs[j].set_radius(Tc[i, j] * TC_SCALE)
            #Tc_circs[j].set_facecolor('red' if Tc[i, j] > 0 else 'blue')
